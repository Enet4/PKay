...
export (float) var thrust_force = 100
# seconds before going berserk
export var patience = 18
export (Rect2) var fly_rect = Rect2(360, 50, 160, 300)
...
func on_spawned(emitter, speed):
	# TODO make connection with paddle for collision detection
...
func _process(delta):
	...
	if self.patience <= 0:
		self.attacking = true
		# increase thrust force a bit
		thrust_force *= 1.5
		# TODO activate kamikaze sound loop
		$AnimatedSprite.animation = "kamikaze"
